...
int main()
{
	...
	time(&beg);
	FILE* log;
	freopen_s(&log, "log.csv", "wb", stdout);
	printf("date,time,srcMAC,srcIP,destMAC,destIP,len\n");
	fclose(log);
	FILE* srcLog;
	freopen_s(&srcLog, "srcLog.csv", "wb", stdout);
	printf("srcLog\n");
	fclose(srcLog);
	FILE* destLog;
	freopen_s(&destLog, "destLog.csv", "wb", stdout);
	printf("destLog\n");
	fclose(destLog);
	/* start the capture */
	...
	if ((adhandle = pcap_open_offline("C:\\Users\\77443\\Desktop\\dns.pcap",			// name of the device
		errbuf					// error buffer
	)) == NULL)
	{
...
void packet_handler(u_char* param, const struct pcap_pkthdr* header, const u_char* pkt_data)
{
	...
	FILE* log;
	freopen_s(&log, "log.csv", "ab", stdout);
	...
	{
		FILE* srcLog;
		freopen_s(&srcLog, "srcLog.csv", "ab", stdout);
		sourceLink.print(cout);
		FILE* destLog;
		freopen_s(&destLog, "destLog.csv", "ab", stdout);
		destLink.print(cout);
		fclose(srcLog);
		fclose(destLog);
		beg = local_tv_sec;
